...
* **Persistent Layout** – Custom dashboards must be saved to `~/.ticker_tape/profiles/<name>.yaml` (or similar).  On application start, the system loads built‑in profiles as well as any custom dashboards defined by the user.
* **Reusable Layout Logic** – Resizing and arrangement logic should reuse the base layout manager (see `ui/layout.py`) and not duplicate code.  Custom dashboards behave identically to built‑in profiles regarding breakpoints, themes, density and fullscreen modes.
...
* Introduce a `Dashboard` model that encapsulates the layout grid, panel list and metadata.  Provide serialization to/from YAML.
