...
def ensure_signal_before_execution(signal_time: datetime, execution_time: datetime) -> None:
    ...
    if signal_time >= execution_time:
        raise ValueError("Signal timestamp must be strictly before execution time to prevent lookahead bias")


def batch_validate_signals(signals: Iterable[datetime], execution_time: datetime) -> None:
    for s in signals:
